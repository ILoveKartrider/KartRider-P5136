...
namespace KartLibrary.File
{
    ...
    public partial class RhoArchive : IRhoArchive<RhoFolder, RhoFile>
    {
        ...
        private byte[] getData(uint dataIndex, uint key)
        {
            ...
            if ((dataInfo.BlockProperty & RhoBlockProperty.Compressed) != RhoBlockProperty.None)
            {
                using (MemoryStream memStream = new MemoryStream(outData))
                {
                    ...
                    using ZLibStream decompressStream = new ZLibStream(memStream, CompressionMode.Decompress);
                    decompressStream.ReadExactly(outData);
                }
            }
        ...
        private void storeFolderAndFiles(RhoFolder folder, Queue<DataSavingInfo> savingInfo, HashSet<uint> usedIndex, ref int dataOffset, uint outRhoKey)
        {
            ...
            // The legacy client resolves both folders and files with binary
            // search, so the serialized directory table must stay in the
            // same ordinal order used by its wcscmp-style comparator.  The
            // in-memory dictionaries preserve insertion order, which breaks
            // lookup after files are appended to an existing archive.
            List<RhoFolder> subFoldersList = new List<RhoFolder>(folder.Folders);
            subFoldersList.Sort((left, right) => string.CompareOrdinal(left.Name, right.Name));

            // Encode folder metadata (without writing yet)
            using (MemoryStream memStream = new MemoryStream())
            {
                BinaryWriter memWriter = new BinaryWriter(memStream);
                List<RhoFile> subFiles = new List<RhoFile>(folder.Files);
                subFiles.Sort((left, right) =>
                {
                    int nameComparison = string.CompareOrdinal(left.NameWithoutExt, right.NameWithoutExt);
                    return nameComparison != 0
                        ? nameComparison
                        : string.CompareOrdinal(Path.GetExtension(left.Name), Path.GetExtension(right.Name));
                });
                ...
                foreach (RhoFile subFile in subFiles)
                {
                    ...
                    if (subFile.FileEncryptionProperty == RhoFileProperty.CompressedEncrypted || subFile.FileEncryptionProperty == RhoFileProperty.Compressed)
                    {
                        ...
                        {
                            using (var compressStream = new ZLibStream(ms, CompressionLevel.SmallestSize, true))
                            {
