...
internal class TimeUtil
{
	...
	static TimeUtil()
	{
		...
		_monthTable = new ushort[13]
		{
			0, 31, 59, 90, 120, 151, 181, 212, 243, 273,
			304, 334, 365
		};
		_monthLeapYearTable = new ushort[13]
		{
	...
	public static bool IsInYear(string period, DateTime time)
	{
		...
		if (!(((array[0] == "*") ? DateTime.MinValue : XmlConvert.ToDateTime(array[0], XmlDateTimeSerializationMode.RoundtripKind)) >= time))
		{
			return time < ((array[1] == "*") ? DateTime.MaxValue : XmlConvert.ToDateTime(array[1], XmlDateTimeSerializationMode.RoundtripKind));
		}
		return false;
	}
