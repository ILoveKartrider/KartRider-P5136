...
internal static class KRPacketCrypto
{
	public static uint HashDecrypt(byte[] pData, uint nLength, uint nKey)
	{
		...
		uint num6 = 0u;
		int num7 = 0;
		for (num7 = 0; (ulong)num7 < (ulong)(nLength >> 4); num7++)
		{
			...
			num6 = num6 ^ BitConverter.ToUInt32(pData, num5 + 12) ^ BitConverter.ToUInt32(pData, num5 + 8) ^ BitConverter.ToUInt32(pData, num5 + 4) ^ BitConverter.ToUInt32(pData, num5);
			num5 += 16;
		}
		num7 *= 16;
		num5 = 0;
		...
		while ((ulong)num7 < (ulong)nLength)
		{
			pData[num7] ^= array[num5];
			num6 = (uint)(num6 ^ (pData[num7] << num5));
			num7++;
			num5++;
		}
		return num6;
