...
internal class KREncodedBlock
{
	...
	public static byte[] Decode(byte[] inputBytes)
	{
		...
		if (inPacket.ReadByte() == 83)
		{
			...
			uint num2 = (((encodeFlag & EncodeFlag.KartCrypto) != 0) ? inPacket.ReadUInt() : 0u);
			uint key = num2;
			uint num3 = (((encodeFlag & EncodeFlag.ZLib) != 0) ? inPacket.ReadUInt() : 0u);
			uint num4 = num3;
			...
			if ((encodeFlag & EncodeFlag.ZLib) != 0)
			{
				...
				if (Adler32Helper.GenerateSimpleAdler32(array) != IPAddress.HostToNetworkOrder(host))
				{
			...
			if ((encodeFlag & EncodeFlag.ZLib) != 0 && num4 != array.Length)
			{
