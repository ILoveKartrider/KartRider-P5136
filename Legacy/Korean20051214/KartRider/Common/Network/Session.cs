...
internal abstract class Session
{
		...
		try
		{
			...
			if (num > 0)
			{
				...
				while (mCursor >= 4)
				{
					...
					if (_RIV != 0)
					{
						if ((_RIV ^ BitConverter.ToUInt32(mBuffer, (int)num2) ^ 0xC9F84A90u) != KRPacketCrypto.HashDecrypt(array, num2 - 4, _RIV))
						{
	...
	public void Send(OutPacket pPacket)
	{
		try
		{
			if (mDisconnected == 0)
			{
				byte[] frame = pPacket.ToArray();
				KartRider.LegacyPacketTrace.LogSend(frame);
				mSendSegments.Enqueue(new ByteArraySegment(frame, pEncrypted: true));
