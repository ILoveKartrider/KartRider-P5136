...
namespace KartRider
{
    public class StartGameData
    {
        ...
        public static void GetKartSpac(OutPacket oPacket, string Nickname, byte StartTimeAttack_SpeedType, ushort KartID, ushort FlyingPetID)
        {
            ...
            WritePost5136KartSpec(oPacket, Kart);
            //------------------------------------------------------------------------KartSpac End
        ...
        private static void WritePost5136KartSpec(OutPacket oPacket, KartSpec kart)
        {
            ...
            oPacket.WriteEncUShort((ushort)(kart.defaultEngineType | (kart.EngineType << 8)));
            oPacket.WriteEncUShort((ushort)(kart.defaultHandleType | (kart.HandleType << 8)));
            oPacket.WriteEncUShort((ushort)(kart.defaultWheelType | (kart.WheelType << 8)));
            oPacket.WriteEncUShort((ushort)(kart.defaultBoosterType | (kart.BoosterType << 8)));
        ...
        public static void KartSpecLog(OutPacket oPacket, int StartPosition)
        {
            ...
            ushort engineTypes = iPacket.ReadEncodedUShort();
            ushort handleTypes = iPacket.ReadEncodedUShort();
            ushort wheelTypes = iPacket.ReadEncodedUShort();
            ushort boosterTypes = iPacket.ReadEncodedUShort();
            Console.WriteLine($"defaultEngineType:{(byte)engineTypes}");
            Console.WriteLine($"EngineType:{(byte)(engineTypes >> 8)}");
            Console.WriteLine($"defaultHandleType:{(byte)handleTypes}");
            Console.WriteLine($"HandleType:{(byte)(handleTypes >> 8)}");
            Console.WriteLine($"defaultWheelType:{(byte)wheelTypes}");
            Console.WriteLine($"WheelType:{(byte)(wheelTypes >> 8)}");
            Console.WriteLine($"defaultBoosterType:{(byte)boosterTypes}");
            Console.WriteLine($"BoosterType:{(byte)(boosterTypes >> 8)}");
