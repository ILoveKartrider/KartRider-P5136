...
namespace KartRider
{
    ...
    public class RandomTrack
    {
        ...
        private static bool IsRandomTrackSelection(uint track)
        {
            return track == 0 ||
                   track == 1 ||
                   track == 3 ||
        ...
        {
            // The P5136 client hashes a concrete track locally before sending
            // GrChangeTrackPacket. Its special/random choices use the small
            // sentinel values above. P5136 intentionally skips the mutating
            // RHO dump, so TrackList is empty in the standalone server; making
            // an exact selection depend on that list incorrectly replaces it
            // with village_R01 at race start.
            if (ClientBuildProfiles.Active.Build == ClientBuild.Korean5136 &&
                !IsRandomTrackSelection(Track))
            {
                Console.WriteLine(
                    "[RandomTrack] P5136 exact selection: 0x{0:X8}",
                    Track);
                return Track;
            }
