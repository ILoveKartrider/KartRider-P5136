...
class MemoryModifier
{
    ...
    public void LaunchAndModifyMemory(string kartRiderDirectory, string pinFile, string pinFileBak)
    {
        ...
        if (!profile.UsesModernPin)
            throw new InvalidOperationException($"{profile.DisplayName} must use the legacy profile launch strategy.");

        ushort loginPort = profile.Ports.ResolveLoginTcpPort(ProfileService.SettingConfig.ServerPort);
            ...
            if (pinFileObj.AuthMethods != null)
            {
                foreach (PINFile.AuthMethod authMethod in pinFileObj.AuthMethods)
                {
                    ...
                    authMethod.LoginServers?.Add(new PINFile.IPEndPoint
                    {
                        ...
                        Port = ClientBuildProfiles.Active.Ports.ResolveLoginTcpPort(ProfileService.SettingConfig.ServerPort)
                    });
                }
            }
